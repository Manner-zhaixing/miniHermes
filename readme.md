...
- **多会话并行** — 桌面端不同会话可同时对话互不排队，切走再切回流式内容完整保留
- **会话绑定工作目录** — 桌面端每个会话绑定独立工作目录，侧栏按目录分组（分区头 + 嵌套会话：项目名/父路径/计数分区头，会话缩进带左竖线，与项目目录一眼区分）；已有消息的会话锁定切换目录，空会话切目录自动跟随
- **对话渲染优化** — 桌面端流式输出舒适排版：单换行保留为换行、行距 1.7、段落间距放宽、标题层级梯度、代码块语法高亮
